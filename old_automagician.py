...
def delpwd():
  ...
  Errors:
    None exist
  Prints
    conformation that the direcory was deleted
  Tests
    /home/jw53959/project/unitTests.py"""
  pwd = os.getcwd()
...
def get_machine_number():
  """Returns the number of the machile that is currently logged in.
  Args:
    None
  Returns:
    int: The number conected with the currrently logged on machine
      if logged on to fri.oden.utexas.edu returns 0
      if logged on to halifax.oden.utexas.edu returns 1
      if logged on to stempede2.tacc.utexas.edu returns 2
      if logged on to fronterra.tacc.utexas.edu returns 3
      if logeed on to ls6.tacc.utexas.edu returns 4
      returns -1 otherwise
  Modifies:
  Tests:
...
def ssh_scp_init():
  ...
  Modifies:
    ssh
      Sets to be a fabric Connection with warn set to True
    scp
      Sets to be a fabric ssh Transfer
    no_ssh (bool)
      Set to True iff fabric was not able to be imported, or
      fri-halifax keys were not present or,
      parser.values.balance == False
  Tests:
    TODO
    Issue - Dont have fri-halifax keys, so ¯\_(ツ)_/¯
  """
  global ssh
  global scp
  global no_ssh
...
def get_subfile(machine):
  ...
  Args:
    machine (int): The machine to get the subfile for,
      0 means fri.oden.utexas.edu
      1 means halifax.oden.utexas.edu
      2 means stampede2.tacc.utexas.edu
      3 means frontera.tacc.utexas.edu
      4 means ls6.tacc.utexas.edu
  Returns:
    str: The correct subfile for the machine
    If the machine is not between 0-4 returns None
  Tests:
    None planned"""
  return {
    0: "fri.sub",
...
def grep_ll_out_convergence(ll_out):
  ...
  Tests:
    None planned"""
  grep_retcode = subprocess.call(["grep", "reached required accuracy - stopping structural energy minimisation", ll_out], stdout= subprocess.DEVNULL)
...
def determine_convergence(job_directory):
  ...
  Changes:
    Changes curent directory to job_directory (why?)
      Im not testing for this.
...
def determine_box_convergence(job_directory):
  ...
  Returns:
    bool: True if there is exactly 1 line in the box relaxation, false otherwise
  Tests:
    Complete"""
  ...
  if (line_number == 0):
    ...
    sprint("number of lines in fe.dat more than 1")
    return False
...
def is_isif3(job_directory):
  ...
  Returns:
    True if the INCAR has ISIF = 3 (spaces ignored), false otherwise
  Tests:
    Tested
  """
...
def process_opt(job_directory):
  ...
  Returns:
    None
  Updates:
    removes convergence certificate if clear certificate is passed by parser
    Updates the job last on
  Tests:
    TODO
    """
...
def archive_ll_out():
  """Moves ll.out to archive_stdout
  Args:
    None
  Returns:
    None
...
def archive_converged(home):
  """renames converged_jobs.dat to archived_converged.dat
  Args:
...
def create_dos_from_sc(job_directory):
  ...
  Changes:
    Submits the job in the job_directory
    Creates a sc subdirectory inside jobdirectory that is set up for a job
    Chages current working directory to job direcoty
  Tests:
    TODO"""
  cwd = os.getcwd()
  ...
  cwd = os.getcwd()
  # TODO is ^ what is wanted, or was the idea to change back?

def sc_is_complete(sc_dir):
  """Returns true if the CHGCAR exists in sc_dir, and has not been modified in over 2 minutes, false otherwise
  Args:
    sc_dir (str): The directory with a sc job to check if is complete
  Returns:
    True if the CHGCAR has not been writen to for 2 minutes, False otherwise, or if CHGCAR does not exist
  Tests
    TODO"""
  cwd = os.getcwd()
  os.chdir(sc_dir)
...
def dos_is_complete(dos_dir):
  """Returns true if the DOSCAR exists in dos_dir, and has not been modified in over 2 minutes, false otherwise
  Args:
    dos_dir (str): The directory with a dos job to check if is complete
  Returns:
    True if the DOSCAR has not been writen to for 2 minutes, False otherwise, or if DOSCAR does not exist
  Tests
    TODO"""
  cwd = os.getcwd()
  os.chdir(dos_dir)
...
def process_dos(job_directory):
  ...
  if opt_jobs[job_directory].status != JobStatus.Converged: # make parent converge first
    return
...
def wav_is_complete(wav_dir):
  ...
  if exists("WAVECAR"):
    ...
    # Why not return something like 2 in this case, so we have diffrent exit codes
    os.chdir(cwd)
    return False
...
def process_wav(job_directory):
  ...
  if os.path.isdir(wav_dir):
    if wav_is_complete(wav_dir):
      ...
      #wav_jobs[job_directory].wav_status = 0
      wav_jobs[job_directory].wav_status = JobStatus.Converged
      #TODO: Why? Was this an error, should this not be JobStatus.Error
  else:
...
def add_preliminary_results(job_directory,step,force,energy):
  """Adds the job directory, step number, force, and enrgy to the file in preliminary_results"""
...
def combine_XDAT_FE(job_directory):
  """Combines XDATCAR and fe.dat from multiple runs and places result in cmbXDATCAR and cmbFE.dat"""
  os.chdir(job_directory)
  ...
  if job_type in ["dos","sc"]:
    ...
    if opt_dir not in dos_jobs:
      if is_remote:
        ...
        dos_jobs[opt_dir] = dos_job(-1, JobStatus.Running, JobStatus.Running, machine, machine)
    ...
    if opt_dir not in wav_jobs:
      if is_remote:
        ...
        wav_jobs[job_dir] = wav_job(-1, JobStatus.Running, machine)
...
def get_submitted_jobs_slurm():
  """Gets all submitted jobs and updates status to -1, or to 2 if there was an error with the job"""
...
def get_submitted_jobs():
  """Updates status to be Positive  for all jobs and get submitted jobs"""
  if machine == 0: # fri
    ...
    for job_dir in dos_jobs:
      ...
      dos_jobs[job_dir].sc_status = dos_jobs[job_dir].sc_status
      dos_jobs[job_dir].dos_status = dos_jobs[job_dir].dos_status
...
def wrap_up(job_directory):
  ...
  Note also moves ll_out to this new run directory
  """
...
def qsub(job_directory):
  ...
  Changes global variable hit_limit
  ...
  with open(subfile_name,"w") as script:
    if machine < 2:
      for line in script_lines:
        if "-N" in line:
          script.write("#$ -N " + "AM_" + os.getcwd().replace("/","_") + "\n")
        else:
          script.write(line)
    else:
      for line in script_lines:
...
def read_job_statuses():
  ...
  for job in db.execute('select * from dos_jobs'):
    ...
    dos_jobs[opt_dir] = dos_job(job[0], JobStatus(job[1]), JobStatus(job[2]), job[3], job[4])

  for job in db.execute('select * from wav_jobs'):
    opt_id = job[0]
    opt_dir = get_string_from_db('select dir from opt_jobs where rowid = ' + str(opt_id))
    wav_jobs[opt_dir] = wav_job(job[0], JobStatus(job[1]), job[2])
...
def write_job_statuses():
  ...
  wav_jobs
  If the job does not have an opt_id  in opt_jobs terminates the program and closes ssh connections
  Updates or inserts job into database depending if it has an entry
  For wav_jobs it selects the old entry form dos_jobs, for undertimed reason
...
def main():
  ...
  What does this do in the "happy path"
    Sets up ssh / scp
    Creates a lockfile
    Initalizes database
    Reads values from database
    Finds the jobs that have been submitted


    Based on the flags this either
      Resets converged
      Archives Converged
      Resets jobs status
  ...
  How does it change state
    home
      sets home to /home for ODEN machines
      Unknown behavior for TACC machines
    prelimary_results
      Ties it to the file at home/preliminary_results.dat.
      Creates the file if it does not already exist.
      Overwrites what was already in the file.
    subfile
      sets subfile to be the correct name for the currently logged on machine.
    machine
      Sets machine to be the number of the currently logged on machine.

  What does it do in the "unhappy path"
    If interupped
      Submits quene
      Deletes lock
      Writes databases to db
  """
  global home
  global preliminary_results
  global subfile
  ...
  try:
    ...
    try:
      ...
      if parser.values.process:
        ...
        for direc in db.execute('select dir from opt_jobs where status = ?', JobStatus.Incomplete):
